module FormsHelper
  ...
  def submit_button(text="Save", opts = {})
    ...
    content_tag :button, opts.merge(type: :submit, class: "btn btn-#{size} btn-primary btn-login", data: { disable_with: disable_with_text(opts.delete(:feedback_text)) }) do
      icon 'fas', icon, text
    end
  end
  ...
  private
    def disable_with_text(feedback_text)
      feedback_text = 'Logging in...' if feedback_text.blank?
